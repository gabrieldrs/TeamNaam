...
exports.run = HG = {
	 ...
	 */
	hungarianAlgortithm: function(matrix) {
		HG.init(matrix);
		// Step 1
		...
		return HG.getSolution(HG.skillMatrix)
	},
	
	init: function(matrix) {
		HG.cols = matrix[0].length;
		HG.rows = matrix.length;
		HG.dim = Math.max(HG.rows, HG.cols);
		HG.solutions = HG.dim;
		HG.skillMatrix = HG.initMatrix(HG.rows, HG.cols);
		HG.matrix = matrix;
		HG.stars = HG.initMatrix(HG.dim, HG.dim);
		HG.matrix = HG.loadMatrix(matrix, true);
		HG.skillMatrix = HG.loadMatrix(matrix, false);
	...
	loadMatrix: function(matrix, reverse) {
		...
		if (reverse) {
			...
			result = HG.reverseMatrix(HG.findMaxValue(result), result);
		}
		return result;
	},
