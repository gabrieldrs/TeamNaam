...
function updateMentor(req, res) {
	...
	Cohort.findById(cid).lean().exec(function( err, cohort){
		...
		if ( cohort && cohort.secret==secret ){
			...
			formData = _.filter( formData, function(formField){ return formField.mentor!==false; });  // only display the form fields relevant to students
...
function updateStudent(req, res) {
	...
	Cohort.findById(cid).lean().exec(function( err, cohort){
		if ( cohort && cohort.secret==secret ){
			var formData = formLoader.getForm(cohort.form);
			formData = _.filter( formData, function(formField){ return formField.student!==false; });  // only display the form fields relevant to students
...
// Posts the result of the updated mentor's application
exports.postUpdateMentor = function(req, res) {
